...
description: Map a company's story to its five valuation drivers.
version: 1.0.0
...
metadata:
  hermes:
    tags: [Valuation, Narrative, Life Cycle, TAM, Corporate Finance]
    related_skills: [readability-check, slop-detector]
---
# Business Narrative Builder

Builds a structured narrative that links a company's qualitative business story to the five quantitative valuation drivers: revenue growth, target operating margin, reinvestment efficiency, cost of capital, and failure risk. Along the way it classifies the company within a six-stage corporate life cycle and sizes the total addressable market. It does not run the discounted cash flow itself; it produces the driver table a DCF consumes.

## When to Use

- Starting a company analysis and the story behind the numbers has not been written down.
- Building a valuation narrative that has to justify each input of a DCF.
- Assessing a company's competitive position or its stage in the corporate life cycle.
- Sizing TAM, SAM, and SOM for a business.
- The user mentions business narrative, story to numbers, life cycle stage, or company analysis.
...
Collect: industry, current revenues, operating income, invested capital, products/services, competitive landscape, geographic breakdown, company age/stage. See [templates/template.md](templates/template.md#company-context-questionnaire) for the context questionnaire.

**Step 2: Classify life cycle stage**

Place the company in one of 6 stages (Start-up, Young Growth, High Growth, Mature Growth, Mature Stable, Decline). Each stage has distinct characteristics for revenue growth, earnings, funding, and competitive dynamics. See [references/methodology.md](references/methodology.md#6-stage-corporate-life-cycle-model) for the full stage definitions and classification criteria.

**Step 3: Size total addressable market**

Estimate TAM using top-down (total market filtered to addressable segment) and bottom-up (unit count times price). Distinguish between TAM (total), SAM (serviceable), and SOM (obtainable). See [templates/template.md](templates/template.md#tam-sizing-template) for the sizing template.

**Step 4: Develop business narrative**

Write a narrative describing how the business evolves over time. The narrative should answer: What market does the company operate in? How big can it get? What are its competitive advantages? How will it make money? See [references/methodology.md](references/methodology.md#narrative-to-numbers-framework) for the 5-step narrative process.

**Step 5: Convert narrative to value drivers**

Translate the narrative into the five quantitative drivers: (1) revenue growth rate and path, (2) target operating margin, (3) reinvestment efficiency (sales-to-capital), (4) risk profile (cost of capital), and (5) failure probability. Each number should trace back to a specific element of the story. See [templates/template.md](templates/template.md#value-driver-mapping-table) for the driver mapping table.

**Step 6: Validate narrative plausibility**

Test whether the narrative is possible (could it happen?), plausible (is it reasonable given evidence?), and probable (is it the most likely outcome?). Develop at least one alternative narrative. Validate using [assets/evaluators/rubric_business_narrative_builder.json](assets/evaluators/rubric_business_narrative_builder.json). Minimum standard: average score of 3.5 or above.
...
Value of Firm = Sum of [FCFF_t / (1 + WACC)^t] + Terminal Value / (1 + WACC)^n
  (detailed DCF mechanics belong to the profile's DCF skill, where it ships one)
```
...
- **[templates/template.md](templates/template.md)**: Company context questionnaire, life cycle classification checklist, TAM sizing template, narrative document template, value driver mapping table
- **[references/methodology.md](references/methodology.md)**: 6-stage life cycle model detail, Damodaran's 5-step narrative-to-numbers framework, narrative plausibility testing, competitive advantage assessment, TAM methodology
- **[assets/evaluators/rubric_business_narrative_builder.json](assets/evaluators/rubric_business_narrative_builder.json)**: Quality criteria for narrative clarity, life cycle classification, TAM sizing, driver linkage, plausibility
...
The narrative is done when each of the five drivers in the mapping table cites the sentence of the story it comes from, the 10-year revenue path implies a market share below 30-40% of the sized TAM, the terminal growth rate is at or below nominal GDP growth, at least one alternative narrative is written out with its own driver values, and the rubric in `assets/evaluators/rubric_business_narrative_builder.json` averages 3.5 or higher.
