...
teams/<team>-team.yaml           manifest for a Bot team: orchestrator, members, tiers, skills
  ...
  config.yaml                    behavioural defaults only; never a model/provider pin
...
tools/                           validate.py, sync_shared.py, build_team.py, team_models.py,
                                 install.sh, publish_mirrors.sh, convert_claude_skill.py
```
...
- Tiers (`orchestrator`/`strong`/`fast`) mirror the work each agent does; packages
  still pin no model. `tools/team_models.py --preset frontier|balanced|budget` applies
