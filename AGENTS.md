...
teams/<team>-team.yaml           manifest for a Bot team: orchestrator, members, tiers, skills
...
tools/                           validate.py, sync_shared.py, build_team.py, team_models.py,
                                 ...
                                 convert_claude_skill.py, memory_setup.py, local_llm.sh
infra/honcho/, infra/launchd/    Honcho env template and a macOS LaunchAgent for the local model server
...
root; **no symlinks anywhere**; nothing named like runtime state (`.env`,
`auth.json`, `memories/`, `sessions/`, `logs/`, `cache/`, `local/` ...).
...
- `description`: **at most 60 characters**, one sentence, capability first,
  ...
  repeat the name. Quote it if it contains a colon. Hermes shows only the
  first 57 characters in the skill index; the trigger must fit.
...
- Canonical sources are the stage briefs in
  `packages/valuation-suite/skills/valuation-specialists/` and the finance skills in
  `packages/valuation-suite/skills/corporate-finance/`. Edit those, then rebuild.
...
Memory is user-owned and never ships in a package. `tools/memory_setup.py` stands up a
self-hosted Honcho stack whose LLM jobs run on a local model (`tools/local_llm.sh`),
