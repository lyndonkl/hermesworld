...
description: Structures content and navigation for findability.
version: 1.0.0
...
metadata:
  hermes:
    category: design
    tags: [Information Architecture, Navigation, Taxonomy, Card Sorting, Tree Testing, Findability]
    related_skills: [cognitive-design, design-evaluation-audit]
---
# Information Architecture

Organizes, structures, and labels content so people can find and manage information: content audits, card sorting, taxonomy and facet design, navigation depth, labeling systems, and tree-testing validation. It produces sitemaps, taxonomies, and test scripts backed by user research rather than stakeholder opinion. It does not design the visual layer or write the copy; it decides where things live and what they are called.

## When to Use

- The user mentions information architecture, IA, sitemap, navigation design, content structure, card sorting, tree testing, taxonomy, or findability.
- Organizing content for a digital product, designing a navigation system, or restructuring an existing information hierarchy.
- Improving findability: users cannot locate content, search logs show failed queries, or bounce rates are high.
- Creating taxonomies, facets, or metadata schemas that must still work when the content grows tenfold.
- Validating a proposed structure with tree testing before design and development spend begins.

Load any reference file with `skill_view("information-architecture", file_path="references/<file>.md")`; the paths below are relative to this skill.
...
An IA is done when it survives contact with users, not when the sitemap looks tidy:

- Tree test success rate of 70% or higher and directness of 1.5x the optimal path or better on the top tasks (see [Success Metrics](#success-metrics)).
- Score the structure against [assets/evaluators/rubric_information_architecture.json](assets/evaluators/rubric_information_architecture.json): ten criteria on a 1-5 scale, minimum average 3.5, target 4.0. Any criterion below 3 (most often User Research Validation or Tree Testing Validation) means the structure is still a hypothesis.

---

## Reference Files

- [Templates](templates/template.md): content audit template, card sorting template, sitemap template, tree testing script
- [Methodology](references/methodology.md): card sorting analysis, taxonomy design, navigation patterns, findability optimization
- [Rubric](assets/evaluators/rubric_information_architecture.json): evaluation criteria for IA quality (10 criteria)

### Related Skills

- `cognitive-design`: the chunking, recognition-over-recall, and Hick's Law reasoning behind the depth and breadth limits here
- `design-evaluation-audit`: reviewing the finished navigation against the 8-dimension cognitive checklist
- `visual-storytelling-design`: presenting the IA decisions and test results to stakeholders
