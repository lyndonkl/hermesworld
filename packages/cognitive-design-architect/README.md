...
Then start a session with `hermes -p cognitive-design-architect chat` (or `cognitive-design-architect chat` if the alias was created). The package pins no model or provider; the installer seeds the model block from your root profile, and `hermes -p cognitive-design-architect model` changes it.
...
- No model tier is pinned; Hermes takes the model from the profile's config.
