...
For every member in teams/<team>-team.yaml it writes packages/<member>/ with:
    ...
    config.yaml         reasoning effort by tier; no model pin
...
def _config_yaml(name: str, tier: str, team: str) -> str:
    effort = "high" if tier == "strong" else "medium"
    prior = "opus" if tier == "strong" else "sonnet"
    return f"""# Behavioural defaults for the {name} Bot (generated by tools/build_team.py).
# Tier: {tier} (the Claude Code version ran this agent on {prior}). No model is pinned;
# apply one per tier with `python3 tools/team_models.py {team} --strong <model> --fast <model>`
# or set it directly with `hermes -p {name} model`. Preserved across `hermes profile update`.
...
def _write_package(pkg: Path, member: dict, team: dict, *, is_orchestrator: bool) -> None:
    ...
    (pkg / "config.yaml").write_text(_config_yaml(member["name"], member["tier"], team["team"]), encoding="utf-8")
